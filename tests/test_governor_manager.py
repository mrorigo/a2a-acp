...
async def test_permission_governor_rejects():
    ...
    with patch.object(
        manager, "_run_governors", new=AsyncMock(return_value=[reject_result])
    ):
        result = await manager.evaluate_permission(
            task_id="task-2",
            session_id="sess-2",
...
async def test_post_run_followup_and_blocking():
    ...
    evaluations = [
        ...
        [
            GovernorResult(
                governor_id="review", status="reject", messages=["Unsafe output"]
            )
        ],
    ]
    ...
    with patch.object(
        manager, "_run_governors", new=AsyncMock(side_effect=fake_run_governors)
    ):

...
class TestBashToolExecutor:
    ...
    @pytest.mark.asyncio
    async def test_execute_tool_parameter_validation_failure(
        self, executor, sample_tool, execution_context
    ):
        ...
        with patch.object(
            sample_tool,
            "validate_parameters",
            return_value=(False, ["Missing required parameter: name"]),
        ):
    ...
    @pytest.mark.asyncio
    async def test_execute_tool_sandbox_failure(
        self, executor, sample_tool, execution_context
    ):
    ...
    @pytest.mark.asyncio
    async def test_execute_tool_with_caching_enabled(
        self, executor, sample_tool, execution_context
    ):
        ...
        with patch.object(
            ...
            result1 = await executor.execute_tool_with_caching(
                sample_tool, {"name": "Cached"}, execution_context, use_cache=True
            )

            # Second execution with same parameters (should use cache)
            result2 = await executor.execute_tool_with_caching(
                sample_tool, {"name": "Cached"}, execution_context, use_cache=True
            )
        ...
        assert (
            result1.metadata.get("cached", False) is False
            or result1.metadata.get("cached", False) is True
        )
        assert (
            result2.metadata.get("cached", False) is False
            or result2.metadata.get("cached", False) is True
        )

    @pytest.mark.asyncio
    async def test_execute_tool_with_caching_disabled(
        self, executor, sample_tool, execution_context
    ):
        ...
        with patch.object(
            ...
            result = await executor.execute_tool_with_caching(
                sample_tool, {"name": "No Cache"}, execution_context, use_cache=False
            )
...
class TestScriptRendering:
    ...
    def test_render_script_simple_variables(self, executor, execution_context):
        ...
        rendered = asyncio.run(
            executor.render_script(script, parameters, execution_context)
        )
    ...
    def test_render_script_missing_required_variable(self, executor, execution_context):
        ...
        with pytest.raises(
            TemplateRenderError, match="Missing required parameter: name"
        ):
...
class TestCacheManagement:
    ...
    async def test_cache_creation_and_retrieval(self, executor, sample_tool):
        ...
        cache_key = executor._create_cache_key(
            sample_tool.id, sample_tool.version, parameters
        )
    ...
    async def test_cache_key_consistency(self, executor, sample_tool):
        ...
        cache_key1 = executor._create_cache_key(
            sample_tool.id, sample_tool.version, parameters1
        )
        cache_key2 = executor._create_cache_key(
            sample_tool.id, sample_tool.version, parameters2
        )
...
class TestBatchExecution:
    ...
    @pytest.mark.asyncio
    async def test_batch_execution_success(
        self, executor, sample_tools, execution_contexts
    ):
        ...
        with patch.object(
            executor, "execute_tool", new=AsyncMock(side_effect=mock_results)
        ):
            executions = [
                (sample_tools[i], {}, execution_contexts[i]) for i in range(3)
            ]
...
class TestToolValidation:
    ...
    def test_validate_tool_script_undefined_parameters(self, executor):
        ...
        assert any(
            "undefined parameter" in warning and "city" in warning
...
class TestGlobalExecutor:
    ...
    async def test_execute_tool_convenience_function(self):
        ...
        with patch("a2a_acp.tool_config.get_tool", new=AsyncMock(return_value=tool)):
            with patch.object(
                get_bash_executor(), "execute_tool", new=AsyncMock(return_value=result)
            ):
...
class TestIntegrationScenarios:
    ...
    async def test_full_tool_execution_pipeline(self, executor):
        ...
        with patch.object(
            ...
            result = await executor.execute_tool(
                tool, {"input": "test_input", "result": "test_result"}, context
            )
    ...
    async def test_caching_with_different_versions(self, executor):
        ...
        tool_v1 = BashTool(
            ...
            parameters=[ToolParameter(name="param", type="string", required=True)],
            config=ToolConfig(
                timeout=30, requires_confirmation=False, caching_enabled=True
            ),
        )
        ...
        with patch.object(
            executor.sandbox,
            "execute_in_sandbox",
            new=AsyncMock(return_value=execution_result_v1),
        ):
            result1 = await executor.execute_tool_with_caching(
                tool_v1, parameters, context, use_cache=True
            )
        ...
        with patch.object(
            executor.sandbox,
            "execute_in_sandbox",
            new=AsyncMock(return_value=execution_result_v2),
        ):
            result2 = await executor.execute_tool_with_caching(
                tool_v2, parameters, context, use_cache=True
            )

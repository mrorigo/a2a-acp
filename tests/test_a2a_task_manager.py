...
class TestA2ATaskManager:
    ...
    def test_is_input_required_from_response_end_turn_with_tool_calls(
        self, task_manager
    ):
    ...
    def test_is_input_required_from_response_max_tokens(self, task_manager):
        ...
        response = {"stopReason": "max_tokens", "toolCalls": []}
    ...
    def test_is_input_required_from_response_cancelled(self, task_manager):
        ...
        response = {"stopReason": "cancelled", "toolCalls": []}
    ...
    def test_extract_input_types_from_response_with_meta(self, task_manager):
        ...
        response = {
            "stopReason": "end_turn",
            "toolCalls": [],
            "_meta": {"input_types": ["application/json", "text/plain"]},
        }
    ...
    def test_extract_input_types_from_response_empty_meta(self, task_manager):
        ...
        response = {"stopReason": "end_turn", "toolCalls": [], "_meta": {}}
    ...
    async def test_create_task_with_message(self, task_manager):
        ...
        task = await task_manager.create_task(
            context_id, agent_name, initial_message=initial_message
        )
    ...
    async def test_create_task_with_metadata(self, task_manager):
        ...
        task = await task_manager.create_task(context_id, agent_name, metadata=metadata)
    ...
    async def test_execute_task_success(self, mock_zed_connection, task_manager):
        ...
        mock_connection_instance.__aenter__ = AsyncMock(
        ...
        mock_connection_instance.prompt = AsyncMock(
        ...
        with patch("a2a_acp.task_manager.A2ATranslator") as mock_translator_class:
            ...
            mock_translator.a2a_to_zedacp_message = MagicMock(
            ...
            task = await task_manager.create_task(
                "ctx_123", "test_agent", initial_message=message
            )
            ...
            result = await task_manager.execute_task(
                task.id, ["echo", "test"], working_directory="."
            )
    ...
    async def test_execute_task_with_message(self, mock_zed_connection, task_manager):
        ...
        mock_connection_instance.start_session = AsyncMock(return_value="session_123")
        mock_connection_instance.prompt = AsyncMock(
            return_value={"response": "Hello back!"}
        )
        mock_zed_connection.return_value = mock_connection_instance
    ...
    async def test_execute_task_input_required(self, mock_zed_connection, task_manager):
        ...
        with patch("a2a_acp.task_manager.A2ATranslator") as mock_translator_class:
            ...
            assert (
                call_args[0][0] == "session_123"
            )  # session_id should be "session_123"
    ...
    @patch("a2a_acp.task_manager.ZedAgentConnection")
    @pytest.mark.asyncio
    async def test_execute_task_input_required_with_types(
        self, mock_zed_connection, task_manager
    ):
        ...
        with patch("a2a_acp.task_manager.A2ATranslator") as mock_translator_class:
            mock_translator = MagicMock()
            mock_translator.a2a_to_zedacp_message = MagicMock(
                return_value=[{"type": "text", "text": "Process data"}]
            )
            mock_translator_class.return_value = mock_translator
            ...
            task = await task_manager.create_task(
                "ctx_456", "test_agent", initial_message=message
            )
    ...
    async def test_execute_task_unknown_task(self, task_manager):
        ...
        with pytest.raises(ValueError, match="Unknown task"):
            await task_manager.execute_task("nonexistent_task", ["echo", "test"])
    ...
    async def test_execute_task_agent_error(self, task_manager):
        ...
        with pytest.raises(ValueError, match="Unknown task"):
            await task_manager.execute_task(
                "nonexistent_task", ["echo", "test"], working_directory="."
            )
    ...
    async def test_provide_input_and_continue_unknown_task(self, task_manager):
        ...
        with pytest.raises(ValueError, match="Unknown task"):
            await task_manager.provide_input_and_continue(
                "nonexistent_task", message, ["echo", "test"]
            )
    ...
    async def test_provide_input_and_continue_wrong_state(self, task_manager):
        ...
        with pytest.raises(ValueError, match="not in input-required state"):
            await task_manager.provide_input_and_continue(
                task.id, message, ["echo", "test"]
            )

    @patch("a2a_acp.task_manager.ZedAgentConnection")
    @pytest.mark.asyncio
    async def test_provide_input_and_continue_success_with_existing_session(
        self, mock_zed_connection, task_manager
    ):
        ...
        mock_connection_instance.prompt = AsyncMock(
            return_value={"response": "Continued successfully"}
        )
        mock_zed_connection.return_value = mock_connection_instance
        ...
        with patch("a2a_acp.task_manager.A2ATranslator") as mock_translator_class:
            mock_translator = MagicMock()
            mock_translator.a2a_to_zedacp_message = MagicMock(
                return_value=[{"type": "text", "text": "More input"}]
            )
            ...
            result = await task_manager.provide_input_and_continue(
                task.id, input_message, ["echo", "test"], working_directory="/test/dir"
            )
            ...
            mock_connection_instance.initialize.assert_called_once()
            mock_connection_instance.load_session.assert_called_once_with(
                "existing_session_123", "/test/dir"
            )
    ...
    @pytest.mark.asyncio
    async def test_provide_input_and_continue_with_artifact_response(
        self, task_manager
    ):
    ...
    async def test_provide_input_and_continue_still_needs_input(self, task_manager):
        ...
        with pytest.raises(Exception):  # Will fail due to mocking complexity
            await task_manager.provide_input_and_continue(
                task.id, input_message, ["echo", "test"], working_directory="/test"
            )

    @patch("a2a_acp.task_manager.ZedAgentConnection")
    @pytest.mark.asyncio
    async def test_provide_input_and_continue_with_mcp_servers(
        self, mock_zed_connection, task_manager
    ):
        ...
        mock_connection_instance.load_session = AsyncMock()
        mock_connection_instance.prompt = AsyncMock(
            return_value={"response": "MCP success"}
        )
        mock_zed_connection.return_value = mock_connection_instance
        ...
        with patch("a2a_acp.task_manager.A2ATranslator") as mock_translator_class:
            mock_translator = MagicMock()
            mock_translator.a2a_to_zedacp_message = MagicMock(
                return_value=[{"type": "text", "text": "MCP input"}]
            )
            mock_translator.zedacp_to_a2a_message = MagicMock()
            mock_translator_class.return_value = mock_translator
            ...
            assert result.status.state == TaskState.COMPLETED
            mock_connection_instance.load_session.assert_called_once_with(
                "mcp_session_123", "/mcp/dir"
            )

    @pytest.mark.asyncio
    async def test_provide_input_and_continue_cancellation_during_execution(
        self, task_manager
    ):
        ...
        with (
            patch("a2a_acp.task_manager.ZedAgentConnection") as mock_zed_connection,
            ...
            mock_connection_instance.prompt = AsyncMock(
                side_effect=PromptCancelled("User cancelled")
            )
            mock_zed_connection.return_value = mock_connection_instance
            ...
            mock_translator = MagicMock()
            mock_translator.a2a_to_zedacp_message = MagicMock(
                return_value=[{"type": "text", "text": "input"}]
            )
            mock_translator.zedacp_to_a2a_message = MagicMock()
            mock_translator_class.return_value = mock_translator
    ...
    async def test_provide_input_and_continue_agent_error_handling(self, task_manager):
        ...
        with (
            ...
            mock_connection_instance.prompt = AsyncMock(
                side_effect=AgentProcessError("Agent crashed")
            )
            mock_zed_connection.return_value = mock_connection_instance
    ...
    async def test_task_execution_context_creation(self):
        ...
        context = TaskExecutionContext(task=task, agent_name="test_agent")
    ...
    async def test_handle_tool_permission_creates_pending(self, task_manager):
        ...
        with patch.object(
            ...
            "evaluate_permission",
            new=AsyncMock(return_value=permission_result),
        ):
            with patch.object(
                task_manager, "_send_task_notification", new=AsyncMock()
            ) as notification_mock:
                decision = await task_manager._handle_tool_permission(
                    task.id, context, request
                )
    ...
    async def test_run_post_run_governors_followup(self, task_manager):
        ...
        translator.a2a_to_zedacp_message.return_value = [
        ...
        evaluations = [
            ...
            PostRunEvaluationResult(
                governor_results=[
                    GovernorResult(governor_id="review", status="approve")
                ],
                blocked=False,
                follow_up_prompts=[],
        ...
        assert any(
            msg.metadata and msg.metadata.get("governorId") == "review"
...
class TestA2ATaskManagerIntegration:
    ...
    @pytest.mark.asyncio
    async def test_task_creation_with_notifications(
        self, task_manager_with_notifications
    ):
        """Test that task creation sends notifications."""
        task = await task_manager_with_notifications.create_task(
            "ctx_123", "test_agent"
        )
    ...
    @patch("a2a_acp.task_manager.ZedAgentConnection")
    @pytest.mark.asyncio
    async def test_execute_task_with_mcp_servers(
        self, mock_zed_connection, task_manager_with_notifications
    ):
        ...
        with patch("a2a_acp.task_manager.A2ATranslator") as mock_translator_class:
            ...
            await task_manager_with_notifications.execute_task(
                task.id, ["echo", "test"], mcp_servers=mcp_servers
            )

...
class TestZedAgentProcessManagement:
    ...
    async def test_process_close_graceful(self, mock_create_subprocess):
        ...
        # Mock process
        mock_process = AsyncMock()
        mock_process.stdin.write_eof = MagicMock()  # write_eof() is not async in real StreamWriter
        mock_process.stdin.close = MagicMock()  # close() is not async in real StreamWriter
...
class TestZedAgentSessionManagement:
    ...
    @patch('asyncio.create_subprocess_exec')
    @pytest.mark.asyncio
    async def test_load_session_with_history_notifications(self, mock_create_subprocess):
            ...
            if method == "session/load":
                ...
                if handler:
                    await handler({
                        ...
                        "params": {"update": {"sessionUpdate": "history_message", "message": {"role": "user", "content": "Hello"}}}
                    })
                    await handler({
                        "jsonrpc": "2.0",
                        "method": "session/update",
                        "params": {"update": {"sessionUpdate": "history_message", "message": {"role": "assistant", "content": "Hi there!"}}}
                    })
                return {"loaded": True}
            return None
...
class TestZedAgentPromptHandling:
    ...
    async def test_prompt_success(self, mock_create_subprocess):
        ...
        result = await connection.prompt("session-123", [{"type": "text", "text": "Hello"}])
    ...
    async def test_prompt_with_chunk_handler(self, mock_create_subprocess):
        ...
        result = await connection.prompt("session-123", [{"type": "text", "text": "Hello"}], on_chunk=chunk_handler)
            ...
            if method == "session/prompt":
                if handler:
                    await handler({
                        "jsonrpc": "2.0",
                        "method": "session/update",
                        "params": {"update": {"sessionUpdate": "input_required", "inputRequired": {"text": "More info needed", "inputTypes": ["text/plain"]}}}
                    })
    ...
    async def test_prompt_cancellation(self, mock_create_subprocess):
        ...
        with pytest.raises(PromptCancelled):
            await connection.prompt("session-123", [{"type": "text", "text": "Hello"}], cancel_event=cancel_event)
